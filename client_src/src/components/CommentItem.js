...
class CommentItem extends Component {
  ...
  onDelete(){
    if (window.confirm("Want to delete?")) {
      axios.delete(`http://localhost:3000/api/commentsmodels/${this.props.item.id}`)
        .then(response => {
                        ...
                        <div className="card-content">
                          <div className="row">
                            ...
                            <div className="col s5 m5 right-align">
                              ...
                              <Link to={`/commentsmodels/edit/${this.props.item.id}`}>Edit</Link>
                                <span>&nbsp;|&nbsp;</span>
                                <a href="#!" onClick={this.onDelete.bind(this)} >Delete</a>
                              </div>
                            </div>
                          </div>
                          <div className="formatted-links">{this.props.item.comment}</div>
                          <div className="formatted-links"><a href="#!" onClick={() => this.setState({showCommentForm: true})}>Reply</a></div>
                        </div>
                    </div>
                  </div>
                </div>
